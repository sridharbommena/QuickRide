...
//    GET /api/trips/{id} — get trip by ID
    @GetMapping("{id}")
    public TripResponseDto getTripById(@PathVariable("id") Long id){
        return tripService.getTripById(id);
    }

//    PATCH /api/trips/{id}/status/{status}
    @PatchMapping("{id}/status/{status}")
    public TripResponseDto updateTrip(@PathVariable("id") Long id, @PathVariable("status")TRIP_STATUS status) throws DriverNotFoundException {
